...
const ctx=vm.createContext({cfg,THREE,BoatDynamics:{...D,chooseRoute:(a,f,length,dt)=>D.chooseRoute(a,f,length,dt,math.random)},ScreenSplash,document,window:{},innerWidth:1280,innerHeight:720,devicePixelRatio:1,addEventListener(){},requestAnimationFrame(){},setTimeout(){},location:{search:''},URLSearchParams,performance,console,Math:math});
...
return test(`(()=>{
 ...
 for(let i=0;i<230*60;i++){
  ...
  let desired=player.lap===1&&player.prog<.13?-10:(cfg.inner||70);
  if(d<(f.kind==='boost'&&f.lat<0?400:(cfg.lead||260))){
   desired=f.kind==='bridge'?f.openings.reduce((best,o)=>Math.abs(o-lane)<Math.abs(best-lane)?o:best)+12:f.kind==='boost'?f.lat+8:Math.abs(f.lat-desired)<20?(f.lat+22<=70?f.lat+22:f.lat-16):desired;
   if(f.kind==='boost'&&Math.floor(goal/features.length)*3+boosts.indexOf(f)===cfg.skip)desired=f.lat>45?f.lat-24:62;
  }
  ...
  keys.s=!!cfg.brake&&raceTime>80&&raceTime<81;
...
console.log('PASS: all three strongest-opponent grid slots narrowly beatable; each omitted boost, wider route and one-second braking error lose.');
